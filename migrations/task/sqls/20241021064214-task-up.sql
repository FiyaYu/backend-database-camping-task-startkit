...
SELECT 
    ...
    (SELECT COUNT(*) FROM "COURSE_BOOKING" WHERE user_id = "CREDIT_PURCHASE".user_id AND status NOT IN ('即將授課','課程已取消'))) 
    AS remaining_credit
FROM "CREDIT_PURCHASE" 
WHERE "CREDIT_PURCHASE".user_id = (SELECT id FROM "USER" WHERE email = 'wXlTq@hexschooltest.io') 
GROUP BY "CREDIT_PURCHASE".user_id;
...
WHERE "COACH_LINK_SKILL".skill_id = (SELECT id FROM "SKILL" WHERE name = '重訓') --只找專長是重訓
